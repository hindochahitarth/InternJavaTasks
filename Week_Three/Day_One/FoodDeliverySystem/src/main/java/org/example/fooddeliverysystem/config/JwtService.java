...
public class JwtService {
    ...
    public String extractUsername(String token) {
        //helper method opens token and reads it secret data(claims)
    ...
    //header specify algorithm,payload custom metadata (claims)  and subject and expiry
    ...
    private String buildToken(
        ...
        //key is string because all json keys are double quoted cant have key as integer in json
    ...
    private boolean isTokenExpired(String token) {
        //checks if token expiration time is before than current then return true means token is expired
    ...
    private Claims extractAllClaims(String token) {
        return Jwts//jwts main helper class of open source jjwt by okta tool to process jwt raw string
                .parserBuilder()
    ...
    private Key getSignInKey() {
        byte[] keyBytes = Decoders.BASE64.decode(secretKey);//takes long text based secret key  and cnverts to base64 into array of bytes
